...
const TeacherCard: React.FC<TeacherCardProps> = ({
  ...
  // Локальные состояния для мгновенного отклика (Optimistic UI)
  ...
  const handleLike = async (e: React.MouseEvent) => {
    ...
    try {
      if (!prevLiked) {
        await supabase.from('ads_likes').insert({ ad_id: teacher.id, user_id: user.id });
      } else {
        await supabase.from('ads_likes').delete().eq('ad_id', teacher.id).eq('user_id', user.id);
      }
    } catch (error) {
    ...
    <div className="group bg-white rounded-3xl border border-slate-200 p-2 pb-4 sm:p-6 md:p-6 transition-all duration-500 hover:shadow-[0_20px_50px_rgba(8,112,184,0.12)] hover:-translate-y-2 relative overflow-hidden flex flex-col h-full">
      <div className="flex justify-end">
        ...
        >
          ...
          />
          <span className={`text-xs font-bold ${isLiked ? "text-rose-600" : "text-slate-600"}`}>
            {likesCount}
          </span>
        </button>
      </div>
        ...
        <div className="w-16 h-16 relative shrink-0">
          {teacher.avatar ? (
            <>
              ...
              />
              {!imgLoaded && <div className="absolute inset-0 w-16 h-16 bg-slate-200 animate-pulse rounded-2xl" />}
            </>
          ) : (
            <div className="w-16 h-16 bg-blue-600 rounded-2xl flex items-center justify-center border border-slate-100 shadow-sm">
              <span className="text-white text-xl font-bold">{teacher.name ? teacher.name[0].toUpperCase() : "?"}</span>
            </div>
          )}
        </div>
        <div className="flex-1">
          <h3 className="font-bold text-slate-900 text-lg leading-tight">{teacher.name ?? "Преподаватель"} {teacher.surname ?? ""}</h3>
          <p className="text-blue-600 text-sm font-semibold mt-1 inline-block bg-blue-50 px-2 py-0.5 rounded-md">{teacher.subject ?? "Предмет не указан"}</p>
        </div>
      </div>
      ...
      <div className="mt-auto pt-5 border-t border-slate-50 flex justify-between items-center">
        <div>
          ...
          <span className="text-xs font-bold text-slate-400 block uppercase tracking-tighter">за 60 минут</span>
        </div>
